<template>
            ...
            >
                ...
                <p-column field="p_slug" header="Tag" filterField="p_slug" dataType="text" sortable style="min-width: 12rem" :showFilterOperator="false" :showAddButton="false">
                    <template #filter="{ filterModel }">
                        <p-inputtext v-model="filterModel.value" type="text" />
                    </template>
                </p-column>
                <p-column field="p_libelle" header="Libelle" sortable style="min-width: 16rem" :showFilterOperator="false" :showAddButton="false">
                    <template #filter="{ filterModel }">
                        <p-inputtext v-model="filterModel.value" type="text" />
                    </template>
                </p-column>
                <p-column field="p_categorie" header="Categorie" sortable style="min-width: 8rem" :showFilterOperator="false" :showAddButton="false">
                    <template #filter="{ filterModel }">
                        <p-inputtext v-model="filterModel.value" type="text" />
                    </template>
                </p-column>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p-floatlabel class="mb-4 mt-5">
                                <p-inputtext id="p_slug" v-model.trim="permission.p_slug" required="true" autofocus :invalid="submitted && !permission.p_slug" fluid />
                                <label for="p_slug" >Tag</label>
                            </p-floatlabel>
                            <small v-if="submitted && !permission.p_slug" class="text-red-500">Tag obligatoire.</small>
                        </div>
                        <div>
                            <p-floatlabel class="mb-4 mt-5">
                                <p-inputtext id="p_libelle" v-model.trim="permission.p_libelle" required="true" autofocus :invalid="submitted && !permission.p_libelle" fluid />
                                <label for="p_libelle">Libelle</label>
                            </p-floatlabel>
                            <small v-if="submitted && !permission.p_libelle" class="text-red-500">Libelle obligatoire.</small>
                        </div>
                        <div>
                            <p-floatlabel class="mb-4">
                                <p-inputtext id="p_categorie" v-model.trim="permission.p_categorie" required="true" autofocus :invalid="submitted && !permission.p_categorie" fluid />
                                <label for="p_categorie">Categorie</label>
                            </p-floatlabel>
                            <small v-if="submitted && !permission.p_categorie" class="text-red-500">Categorie obligatoire.</small>
                        </div>
                    </div>
                </form>
            ...
            <p-dialog v-model:visible="deletePermissionDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                <div class="flex items-center gap-4">
                    ...
                    <span v-if="permission"
                        >êtes-vous sûr(e) de vouloir supprimer <b>{{ permission.p_libelle }}</b
                        >?</span
                    >
                </div>
                <template #footer>
    ...
    export default {
        ...
        methods:{
            ...
            loadLazyData() {
                ...
                this.$axios.post('/api/admin/permission/getpermissions',{lazyEvent: JSON.stringify( this.lazyParams )})
                    .then(response => {
            ...
            savePermission() {
                ...
                if (this.permission?.p_slug?.trim()
                        && this.permission?.p_libelle?.trim()
                        && this.permission?.p_categorie?.trim()) {
                    this.load_button_save = true;
                    if (this.permission.id) {
                        this.$axios.post('/api/admin/permission/updatepermission',{permission: JSON.stringify( this.permission )})
                            .then(response => {
                                this.loadLazyData();
                        ...
                        this.$axios.post('/api/admin/permission/createpermission',{permission: JSON.stringify( this.permission )})
                            .then(response => {
                                this.loadLazyData();
            ...
            deletePermission(){
                ...
                this.$axios.post('/api/admin/permission/deletepermission',{permission: JSON.stringify( this.permission )})
                            .then(response => {
                                this.loadLazyData();
            ...
            exportCSV() {
                ...
                this.$axios.post('/api/admin/permission/getpermissions',{lazyEvent: JSON.stringify( this.lazyParamsEXPORT )})
                    .then(response => {
            ...
            initFilters() {
                this.filters= {
                    p_slug: {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.CONTAINS}]},
                    p_libelle: {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.CONTAINS}]},
                    p_categorie: {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.CONTAINS}]},
