<template>
            ...
            >
                ...
                <p-column filterMatchMode="in" :showFilterMatchModes="false" field="roles_id" header="Role" style="min-width: 10rem" :showFilterOperator="false" :showAddButton="false">
                    <template #body="{data}">
                        {{data.role.r_libelle}}
                    </template>
                    <template #filter="{ filterModel }">
                        <p-multiselect v-model="filterModel.value" :options="roles" optionValue="id" optionLabel="r_libelle" placeholder="Tous"></p-multiselect>
                    </template>
                </p-column>
            ...
            <p-dialog v-model:visible="userDialog" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" header="Utilisateur" :modal="true">
                <form autocomplete="off">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <p-floatlabel class="mb-4 mt-5">
                                <p-inputtext id="prenom" v-model.trim="user.prenom" required="true" autofocus :invalid="submitted && !user.prenom" fluid />
                        ...
                        <div>
                            <p-floatlabel class="mb-4">
                                <p-inputtext id="username" v-model.trim="user.username" required="true" autofocus :invalid="submitted && !user.username" fluid />
                        ...
                        <div>
                            <p-floatlabel class="mb-4">
                                <p-inputtext id="email" v-model.trim="user.email" required="true" autofocus :invalid="submitted && !user.email" fluid />
                        ...
                        <div>
                            <p-floatlabel class="mb-4">
                                <p-select id="role" v-model="user.role" :options="roles" optionLabel="r_libelle" placeholder="Sélectionnez un role" :invalid="submitted && !user.role" fluid></p-select>
            ...
            <p-dialog v-model:visible="deleteUserDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                <div class="flex items-center gap-4">
                    ...
                    <span v-if="user"
                        >êtes-vous sûr(e) de vouloir supprimer <b>{{ user.username }}</b
                        >?</span
                    >
                </div>
                <template #footer>
...
import { FilterMatchMode, FilterOperator } from '@primevue/core/api';
    export default {
        ...
        methods:{
            initRoles(){
                this.$axios.get('/api/admin/user/getroles')
                    .then(response => {
            ...
            loadLazyData() {
                ...
                this.$axios.post('/api/admin/user/getusers',{lazyEvent: JSON.stringify( this.lazyParams )})
                    .then(response => {
            ...
            saveUser() {
                ...
                if (this.user?.username?.trim()
                    ...
                    if (this.user.id) {
                        this.$axios.post('/api/admin/user/updateuser',{user: JSON.stringify( this.user )})
                            .then(response => {
                                this.loadLazyData();
                        ...
                        &&  this.user?.password == this.user?.cpassword){
                            this.$axios.post('/api/admin/user/createuser',{user: JSON.stringify( this.user )})
                                .then(response => {
                                    this.loadLazyData();
            ...
            savePasswordUser() {
                this.submitted = true;
                    ...
                    if (this.user.id) {
                        ...
                        && this.user?.cpassword
                        &&  this.user?.password == this.user?.cpassword){
                        this.$axios.post('/api/admin/user/updatepassworduser',{user: JSON.stringify( this.user )})
                            .then(response => {
                                this.loadLazyData();
            ...
            deleteUser(){
                ...
                this.$axios.post('/api/admin/user/deleteuser',{user: JSON.stringify( this.user )})
                            .then(response => {
                                this.loadLazyData();
            ...
            exportCSV() {
                ...
                this.$axios.post('/api/admin/user/getusers',{lazyEvent: JSON.stringify( this.lazyParamsEXPORT )})
                    .then(response => {
            ...
            initFilters() {
                this.filters= {
                    ...
                    roles_id: {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.IN}]},
